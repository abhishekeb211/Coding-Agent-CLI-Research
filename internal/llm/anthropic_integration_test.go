...
// TestAnthropicProviderIntegration tests Anthropic provider integration with Manager
func TestAnthropicProviderIntegration(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if reqBody["model"] != "claude-3-sonnet-20240229" {
			t.Errorf("Expected model claude-3-sonnet-20240229, got %v", reqBody["model"])
		}
	...
	// Create manager with Anthropic provider
	config := Config{
		Provider:     "anthropic",
		APIKey:       "test-api-key",
...
// TestAnthropicProviderFallback tests fallback from Anthropic to mock provider
func TestAnthropicProviderFallback(t *testing.T) {
	// Create mock Anthropic server that always fails
	...
	// Create manager with Anthropic as primary and mock as fallback
	config := Config{
		Provider:          "anthropic",
...
// TestAnthropicProviderCostEstimation tests cost estimation
func TestAnthropicProviderCostEstimation(t *testing.T) {
	config := Config{
		Provider:     "anthropic",
		APIKey:       "test-key",
		Model:        "claude-3-sonnet-20240229",
	...
	// Cost should be greater than 0 for Anthropic
	if cost <= 0 {
...
// TestAnthropicProviderCostLimit tests cost limit enforcement
func TestAnthropicProviderCostLimit(t *testing.T) {
	// Create mock Anthropic server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		response := map[string]interface{}{
			"content": []map[string]interface{}{
				{
					"type": "text",
					"text": `{"explanation":"test","remediation_steps":["step1"],"confidence":0.8}`,
				},
			},
		}
		w.Header().Set("Content-Type", "application/json")
...
// TestAnthropicProviderRateLimiting tests rate limiting
func TestAnthropicProviderRateLimiting(t *testing.T) {
...
// TestAnthropicProviderWithCache tests caching with Anthropic provider
func TestAnthropicProviderWithCache(t *testing.T) {
	callCount := 0
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		response := map[string]interface{}{
			"content": []map[string]interface{}{
				{
					"type": "text",
					"text": `{"explanation":"SQL Injection test","remediation_steps":["Use parameterized queries"],"confidence":0.9}`,
				},
			},
		}
		w.Header().Set("Content-Type", "application/json")
...
// TestAnthropicProviderErrorHandling tests various error scenarios
...
// TestAnthropicProviderMultipleModels tests different Claude models
func TestAnthropicProviderMultipleModels(t *testing.T) {
	models := []string{
		...
		t.Run(model, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if reqBody["model"] != model {
					t.Errorf("Expected model %s, got %v", model, reqBody["model"])
				}
